...
if [ -f "mix.exs" ]; then
    ...
    echo ""
    echo "  1. Run ./scripts/start.sh to validate & start development"
    echo "     (Auto-installs missing deps, configures env, starts server)"
    echo "  2. Review and customize CLAUDE.md for your project"
    echo "  3. Configure .claude/settings.json for your team"
    echo "  4. Start development with: /start-discovery"
else
    echo -e "${YELLOW}Next Steps:${NC}"
    echo ""
    echo "  1. Run ./scripts/start.sh --setup for full setup"
    echo "     (Auto-installs Elixir, Node.js, creates Phoenix project)"
    echo "  2. Review and customize CLAUDE.md for your project"
    echo "  3. Configure .claude/settings.json for your team"
    echo "  4. Start development with: /start-discovery"
    echo ""
    print_info "The start.sh script will auto-install missing dependencies!"
fi
echo ""
...
echo -e "${BLUE}Template Resources:${NC}"
echo ""
echo "  .claude/agents/i/     AI agents for specialized tasks"
echo "  .claude/assets/i/     Brand assets + 2600+ SVG icons"
echo "  .claude/commands/i/   Slash commands for workflows"
